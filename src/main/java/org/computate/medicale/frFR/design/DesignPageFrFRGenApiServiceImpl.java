...
import org.computate.medicale.frFR.design.DesignPageFrFRGenApiServiceImpl;
import org.computate.medicale.frFR.design.DesignPage;
import org.computate.medicale.frFR.html.part.PartHtmlFrFRGenApiServiceImpl;
...
/**
 * Traduire: false
 * NomCanonique.enUS: org.computate.medicale.enUS.design.PageDesignEnUSGenApiServiceImpl
 **/
		...
		try {
			sqlDesignPage(requeteSite, a -> {
				if(a.succeeded()) {
					...
					LOGGER.error(String.format("designpdfpagerechercheDesignPage a échoué. ", a.cause()));
					erreurDesignPage(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
		...
		try {
			sqlDesignPage(requeteSite, a -> {
				if(a.succeeded()) {
					...
					LOGGER.error(String.format("designmailpagerechercheDesignPage a échoué. ", a.cause()));
					erreurDesignPage(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
