...
import org.computate.medicale.enUS.cluster.ClusterEnUSGenApiService;
...
//		vertx.setPeriodic(1000 * 60 * 60 * 60, a -> {
	...
	public void  futureAuthorizeNetEnrollmentCharges(MerchantAuthenticationType merchantAuthenticationType, SearchList<Inscriptionmedicale> listSchoolEnrollment, PaiementmedicaleFrFRGenApiServiceImpl paymentService, Handler<AsyncResult<Void>> eventHandler) {
		List<Future> futures = new ArrayList<>();
	...
	public Future<Void> futureAuthorizeNetCharge(MerchantAuthenticationType merchantAuthenticationType, Inscriptionmedicale schoolEnrollment, PaiementmedicaleFrFRGenApiServiceImpl paymentService, SiteRequestEnUS siteRequest, Handler<AsyncResult<Void>> a) {
		Promise<Void> promise = Promise.promise();
		try {
	...
	public Future<Void> futureAuthorizeNetEnrollmentPayments(MerchantAuthenticationType merchantAuthenticationType, Inscriptionmedicale schoolEnrollment, Handler<AsyncResult<Void>> a) {
		Promise<Void> promise = Promise.promise();
		try {
...
//											List<SimpleOrderedMap> enrollmentKeys = (List<SimpleOrderedMap>)enrollmentKeysList.get(0);
	...
	public Future<Void> futureAuthorizeNetPayment(PaiementmedicaleFrFRGenApiServiceImpl paymentService, SiteRequestEnUS siteRequest, TransactionSummaryType transaction, Inscriptionmedicale schoolEnrollment, Handler<AsyncResult<Void>> a) {
		Promise<Void> promise = Promise.promise();
		try {
	...
	private Promise<Void> startServer() {
		...
		if("medicale-dev.computate.org".equals(siteConfig.getSiteHostName())) {
			staticHandler.setAllowRootFileSystemAccess(true);
			staticHandler.setWebRoot("/usr/local/src/computate-medicale-static");
		}
